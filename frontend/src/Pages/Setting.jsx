...
const Setting = () => {
  ...
  const settings = [
    ...
    { category: "Manage", options: [{ name: "Liked", path: "/account/liked-post" }, { name: "Comments", path: "/account/commented-post" }, { name: "Saved", path: "/account/saved-post" }, { name: "Delete Account", path: "/delete-account" }] },
  ...
  return (
    <div className="p-6 bg-base-100">
      <h2 className="text-3xl font-bold text-center mb-6 text-primary">Settings</h2>
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
        {settings.map((section, index) => (
          <div key={index} className="bg-base-200 p-5 rounded-lg shadow-md">
            <h3 className="text-lg font-semibold mb-3 text-base-content">{section.category}</h3>
